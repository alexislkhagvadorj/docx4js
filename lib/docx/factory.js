  ...
  _createClass(Factory, [{
    ...
    value: function create(wXml, doc, parent, more) {
      ...
      if ('document' == tag) return new (require('./model/document'))(wXml, doc, parent);else if ('styles' == tag) return new (require('./model/documentStyles'))(wXml, doc);else if ('abstractNum' == tag) return new (require('./model/style/numberingDefinition'))(wXml, doc);else if ('num' == tag) return new (require('./model/style/list'))(wXml, doc);else if ('style' == tag) {
        ...
        if (wXml.$2('>pPr>numPr') || style && style.getNumId() != -1) return new (require('./model/list'))(wXml, doc, parent);

        var outlineLvl = -1,
            tmp = void 0;
        if (style) outlineLvl = style.getOutlineLevel();else if (tmp = wXml.$2('>pPr>outlineLvl')) {
        ...
        var _style = doc.style.get(attr(wXml.$2('>rPr>rStyle'), 'w:val'));

        var _outlineLvl = -1,
            _tmp = void 0;
        if (_style) _outlineLvl = _style.getOutlineLevel();else if (_tmp = wXml.$2('>rPr>outlineLvl')) {
          _tmp = attr(_tmp);
        ...
        if (elBinding) {
          ...
          //controls
          var elType = wXml.$2('>sdtPr').$2('text, picture, docPartList, comboBox, dropDownList, date, checkbox');
